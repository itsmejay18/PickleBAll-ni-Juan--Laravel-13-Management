        ...
        <div class="card shadow-lg">
            <div class="card-header pb-0">
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
                    <div>
                        <h6 class="mb-0 font-weight-bolder">Rates Management</h6>
                        <p class="text-sm mb-0">Configure standard base rates, peak season rules, and equipment rental rates for all branches.</p>
                    </div>
                </div>

                <!-- Soft UI Styling Pills Navigation -->
                <div class="nav-wrapper position-relative end-0 mb-4">
                    <ul class="nav nav-pills nav-fill p-1 bg-gray-100 border-radius-lg" role="tablist">
                        <li class="nav-item">
                            <a class="nav-link mb-0 px-0 py-1 active" data-bs-toggle="tab" href="#court-rates-pane" role="tab" aria-selected="true">
                                <i class="fas fa-table-tennis text-sm me-2 text-dark"></i>
                                <span class="ms-1">Court Rates</span>
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link mb-0 px-0 py-1" data-bs-toggle="tab" href="#equipment-rates-pane" role="tab" aria-selected="false">
                                <i class="fas fa-boxes text-sm me-2 text-dark"></i>
                                <span class="ms-1">Equipment Rates</span>
                            </a>
                        </li>
                    </ul>
                </div>
            ...
            <div class="card-body px-0 pb-2">
                <div class="tab-content">
                    ...
                    <!-- Tab 1: Court Rates -->
                    <div class="tab-pane fade show active" id="court-rates-pane" role="tabpanel">
                        <div class="table-responsive">
                            <table class="table align-items-center mb-0">
                                <thead>
                                    <tr>
                                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder">Court Name</th>
                                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder">Location</th>
                                        <th class="text-end text-uppercase text-secondary text-xxs font-weight-bolder">Base Hourly Price</th>
                                        <th class="text-end text-uppercase text-secondary text-xxs font-weight-bolder pe-4">Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                            ...
                                            <td>
                                                <div class="d-flex px-3 py-1">
                                                    ...
                                                    <div class="d-flex flex-column justify-content-center">
                                                        <h6 class="mb-0 text-sm font-weight-bold">{{ $court['court_name'] ?: 'Court '.$court['court_number'] }}</h6>
                                                        <p class="text-xs text-secondary mb-0">Number: #{{ $court['court_number'] }}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="text-sm px-3">{{ $court['location_name'] }}</td>
                                            <td class="text-end text-sm font-weight-bold px-3">PHP {{ number_format($court['base_price'], 2) }}</td>
                                            <td class="text-end pe-4">
                                                <button type="button" class="btn btn-sm bg-gradient-primary mb-0 py-1 px-3"
                                                    onclick='pbjOpenRatesModal(@json($court))'>
                                                    <i class="fas fa-tags me-1"></i>Manage Rates
                                                </button>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="4" class="text-center text-sm text-secondary py-5">No courts found.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                    ...
                    <!-- Tab 2: Equipment Rates -->
                    <div class="tab-pane fade" id="equipment-rates-pane" role="tabpanel">
                        <div class="table-responsive">
                            <table class="table align-items-center mb-0">
                                <thead>
                                    <tr>
                                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder">Equipment Item</th>
                                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder">Location</th>
                                        <th class="text-end text-uppercase text-secondary text-xxs font-weight-bolder">Rental Rate</th>
                                        <th class="text-end text-uppercase text-secondary text-xxs font-weight-bolder">Deposit Amount</th>
                                        <th class="text-end text-uppercase text-secondary text-xxs font-weight-bolder pe-4">Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @forelse ($equipment as $item)
                                        <tr>
                                            ...
                                            <td class="text-sm px-3">{{ $item['location_name'] }}</td>
                                            <td class="text-end text-sm font-weight-bold px-3">PHP {{ number_format($item['rental_price_per_unit'], 2) }}</td>
                                            <td class="text-end text-sm px-3">PHP {{ number_format($item['deposit_amount'], 2) }}</td>
                                            <td class="text-end pe-4">
                                                <button type="button" class="btn btn-sm bg-gradient-primary mb-0 py-1 px-3"
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="5" class="text-center text-sm text-secondary py-5">No equipment inventory found.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
...
@push('modals')
    ...
    <div class="modal fade" id="equipmentRatesModal" tabindex="-1" aria-labelledby="equipmentRatesModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <form id="equipmentRatesForm" method="POST">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="reorder_point" id="er_reorder_point">
                    <input type="hidden" name="available_quantity" id="er_available">
                    <input type="hidden" name="damaged_quantity" id="er_damaged">
                    <input type="hidden" name="lost_quantity" id="er_lost">
                    <input type="hidden" name="is_available_for_rent" id="er_rentable" value="1">
                    
                    <div class="modal-header">
                        <h5 class="modal-title" id="equipmentRatesModalLabel">Manage Equipment Rates</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body text-start">
                        <h6 class="text-xs text-uppercase text-secondary font-weight-bolder mb-3" id="er_item_name">Item</h6>
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label class="form-label text-xs">Rental Price (PHP)</label>
                                <input type="number" name="rental_price_per_unit" id="er_price" class="form-control" required min="0" step="0.01">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label text-xs">Deposit Amount (PHP)</label>
                                <input type="number" name="deposit_amount" id="er_deposit" class="form-control" min="0" step="0.01">
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-outline-secondary mb-0" data-bs-dismiss="modal">Cancel</button>
                        <button type="submit" class="btn bg-gradient-primary mb-0">Save Rates</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        ...
        let equipmentRatesModalInstance = null;
        function pbjOpenEquipmentRatesModal(item) {
            if (!item) return;
            const form = document.getElementById('equipmentRatesForm');
            form.action = "{{ url('/equipment') }}/" + item.inventory_id;
            
            document.getElementById('er_item_name').textContent = item.name + ' Rates (' + item.location_name + ')';
            document.getElementById('er_price').value = item.rental_price_per_unit;
            document.getElementById('er_deposit').value = item.deposit_amount;
            
            document.getElementById('er_reorder_point').value = item.reorder_point;
            document.getElementById('er_available').value = item.available_quantity;
            document.getElementById('er_damaged').value = item.damaged_quantity || 0;
            document.getElementById('er_lost').value = item.lost_quantity || 0;
            document.getElementById('er_rentable').value = item.is_available_for_rent ? '1' : '0';
            
            if (!equipmentRatesModalInstance) {
                equipmentRatesModalInstance = new bootstrap.Modal(document.getElementById('equipmentRatesModal'));
            }
            equipmentRatesModalInstance.show();
        }
    </script>
@endpush
